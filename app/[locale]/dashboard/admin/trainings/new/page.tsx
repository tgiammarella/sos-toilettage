      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div>
          <Button variant="ghost" size="sm" asChild className="mb-4 -ml-2">
            <Link href={`/${locale}/dashboard/admin/trainings`}>
              <ArrowLeft className="h-4 w-4 mr-1" />
              {locale === "fr" ? "Retour" : "Back"}
            </Link>
          </Button>
          <h1 className="text-2xl font-bold text-[#1F2933]">
            {locale === "fr" ? "Nouvelle formation" : "New training"}
          </h1>
        </div>
        <TrainingListingForm locale={locale} defaultType="COURSE" defaultIsTrainer={false} />
      </div>
    </AdminShell>
  );
}
